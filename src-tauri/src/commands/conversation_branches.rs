...
use crate::db::service::{
    artifact_service, conversation_branch_service, conversation_branch_task_service,
    conversation_service, folder_service,
};
...
#[serde(rename_all = "camelCase")]
pub struct ConversationBranchCreationTask {
    pub request_id: String,
    pub source_conversation_id: i32,
    pub status: String,
    pub stage: String,
    pub result: Option<CreateConversationBranchResult>,
...
fn branch_creation_task_view(
    row: crate::db::entities::conversation_branch_creation_task::Model,
) -> ConversationBranchCreationTask {
    ConversationBranchCreationTask {
        result: row
            .result_json
            .as_deref()
...
pub async fn queue_conversation_branch_creation_core(
    db: &AppDatabase,
    request: &CreateConversationBranchRequest,
) -> Result<ConversationBranchCreationTask, AppCommandError> {
    let request_id = request
        ...
        .ok_or_else(|| AppCommandError::invalid_input("branch creation request id is required"))?;
    let row = conversation_branch_task_service::create_or_get(
        &db.conn,
        request_id,
    ...
    Ok(branch_creation_task_view(row))
}

pub async fn get_conversation_branch_creation_task_core(
    db: &AppDatabase,
    request_id: &str,
...
pub async fn cancel_conversation_branch_creation_task_core(
    db: &AppDatabase,
    request_id: &str,
) -> Result<Option<ConversationBranchCreationTask>, AppCommandError> {
    Ok(conversation_branch_task_service::cancel(&db.conn, request_id)
        .await
        .map_err(AppCommandError::from)?
        .map(branch_creation_task_view))
}

pub async fn run_conversation_branch_creation_task(
    db: AppDatabase,
    manager: ConnectionManager,
    emitter: EventEmitter,
    data_dir: std::path::PathBuf,
    owner_label: String,
    mut request: CreateConversationBranchRequest,
) {
    ...
    let Ok(true) = conversation_branch_task_service::mark_running(&db.conn, &request_id).await
    else {
        return;
    };
    request.request_id = Some(request_id.clone());
    request.operation_id = Some(request_id.clone());
    let outcome = create_conversation_branch_core(
        &db,
        &manager,
        &emitter,
        &data_dir,
    ...
    if let Err(error) = conversation_branch_task_service::finish(
        &db.conn,
        &request_id,
        result_json,
        error,
    ...
    .await
    {
        tracing::error!(request_id, error = %error, "failed to persist branch task result");
    }
}
...
pub async fn queue_conversation_branch_creation(
    ...
    let task = queue_conversation_branch_creation_core(&db, &request).await?;
    if task.status == "queued" {
        ...
        tokio::spawn(run_conversation_branch_creation_task(
            owned_db,
            owned_manager,
            emitter,
            data_dir,
...
#[cfg(feature = "tauri-runtime")]
#[cfg_attr(feature = "tauri-runtime", tauri::command)]
pub async fn get_conversation_branch_creation_task(
    db: tauri::State<'_, AppDatabase>,
    request_id: String,
) -> Result<Option<ConversationBranchCreationTask>, AppCommandError> {
    get_conversation_branch_creation_task_core(&db, &request_id).await
}

#[cfg(feature = "tauri-runtime")]
#[cfg_attr(feature = "tauri-runtime", tauri::command)]
pub async fn cancel_conversation_branch_creation_task(
    db: tauri::State<'_, AppDatabase>,
    request_id: String,
) -> Result<Option<ConversationBranchCreationTask>, AppCommandError> {
    cancel_conversation_branch_creation_task_core(&db, &request_id).await
}
