...
pub async fn queue_conversation_branch_creation(
    Extension(state): Extension<Arc<AppState>>,
    Json(params): Json<CreateConversationBranchParams>,
) -> Result<Json<branch_commands::ConversationBranchCreationTask>, AppCommandError> {
    let task = branch_commands::queue_conversation_branch_creation_core(&state.db, &params.request)
        .await?;
    if task.status == "queued" {
        let db = crate::db::AppDatabase {
            conn: state.db.conn.clone(),
        };
        tokio::spawn(branch_commands::run_conversation_branch_creation_task(
            db,
            state.connection_manager.clone_ref(),
            state.emitter.clone(),
            state.data_dir.clone(),
            "web:conversation-branch-task".into(),
            params.request,
        ));
    }
    Ok(Json(task))
...
#[derive(Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct ConversationBranchCreationTaskParams {
    pub request_id: String,
}

pub async fn get_conversation_branch_creation_task(
    Extension(state): Extension<Arc<AppState>>,
    Json(params): Json<ConversationBranchCreationTaskParams>,
) -> Result<Json<Option<branch_commands::ConversationBranchCreationTask>>, AppCommandError> {
    Ok(Json(
        branch_commands::get_conversation_branch_creation_task_core(
            &state.db,
            &params.request_id,
        )
        .await?,
...
pub async fn cancel_conversation_branch_creation_task(
    ...
    Ok(Json(
        branch_commands::cancel_conversation_branch_creation_task_core(
            &state.db,
            &params.request_id,
        )
        .await?,
